...
if ($number) {
    if ($s) {
        $page = new apps_libs_Page([
            "table" => "users",
            "where" => "users.user_type=2 or users.user_type=3 or users.user_type=4 and users.user_username LIKE '" . $s . "%' ORDER BY users.user_id ASC",
            "col" => [
                "ID" => "users.user_id",
            ...
            "break" => ["users.user_id", "users.matp","users.user_type"]
        ], $number, $max);
        echo CreateTableA($page);
    } else {
        $page = new apps_libs_Page([
            "table" => "users",
            "where" => "users.user_type=2 or users.user_type=3 or users.user_type=4 ORDER BY users.user_id ASC",
            "col" => [
                "ID" => "users.user_id",
                "Tài Khoản" => "users.user_username",
                "Email" => "users.user_email",
