...
if ($number) {
    if ($s) {
        ...
        $page = new apps_libs_Page([
            "table" => "orders,end_users,products",
            "where" => "products.product_id=orders.order_product and orders.order_user=end_users.end_user_id and end_users.adtudo_id = {$_SESSION['userID']} order by  orders.order_completed ASC,orders.created_at",
            "col" => [
                "ID" => "orders.order_id",
                "Email" => "end_users.end_user_email",
                "Người Mua" => "end_users.end_user_fullname",
...
{   
    ...
    foreach($ret_data as $key => $val):
        //Kiểm tra quyền xử lý nếu k có tự do thì tìm huyện, tỉnh, tổng
    ...
    <?php 
//Vòng lặp lấy chi tiết sản đơn hàng
$i = 1;
