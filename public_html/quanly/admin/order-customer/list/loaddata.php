...
if ($number) {
    if ($s) {
        ...
        $page = new apps_libs_Page([
            "table" => "orders,end_users,products",
            "where" => "products.product_id=orders.order_product and orders.order_user=end_users.end_user_id order by  orders.order_completed ASC,orders.created_at",
            "col" => [
                "ID" => "orders.order_id",
                "Email" => "end_users.end_user_email",
                "Người Mua" => "end_users.end_user_fullname",
    ...
    foreach($ret_data as $key => $val):
        switch ($val[0]['order_type']) {
            ...
            case 3:
                ...
                break;
            case 2:
                $sql = "select * from users where user_type = 2 and matp = {$val[0]['end_user_city']} ";
                $admin = $db->query($sql);
                if(!empty($admin))
                    $ret_data[$key]['admin'] = $admin[0];
                    $ret_data[$key]['admin']->admin_type = 'Admin tỉnh';
                break;
