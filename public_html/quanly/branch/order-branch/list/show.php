...
<div class="h-padding">
                    ...
                    <div class="form-group"> 
                        <div class="col-sm-12" style="padding-bottom: 20px ">
                            <div style="float: left">
                                ...
                                <span style="color: red;font-size: 26px" id="total"></span>
                             </div>
                            <input style="float: right;margin-top: 10px" onclick="send_ajax()" type="button" id="submit" class="btn btn-primary" value="Đặt Hàng"/>
                        </div>
                    </div>  
                </div>
            </div>
    ...
    <script>
        $(document).ready(function(){
            setTimeout(function(){
                $('.count-sp').change(function(){
                var tongtien = 0;
                $('.count-sp').each(function(){
                    gia = $(this).parent('td').prev().html();
                    sluong = $(this).val();
                    tongtien += gia * sluong
                });
                $('#total').html(new Intl.NumberFormat('en-US').format(tongtien));
        ...
        function send_ajax() {
            ...
            $.ajax({
                url: "order-branch/list/send-order.php",
                type: "post",
                dataType: "json",
                data: {
                    submit:$('#submit').val(),
        ...
        function load_ajax(number, value = null, max = null) {
            ...
            $.ajax({
                url: "order-branch/list/loaddata.php",
                type: "post",
                dataType: "text",
                data: {
                    number: number,
