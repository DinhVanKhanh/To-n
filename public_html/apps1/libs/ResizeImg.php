...
class apps_libs_ResizeImg
{
    private $img_in;
    private $img_out;
    function apps_libs_ResizeImg($img_in, $img_out)
    {
        $this->img_in = $img_in;
        $this->img_out = $img_out;
    }

    private function resize_image_crop($image, $width, $height)
    {
    ...
    private function resize_image_max($image, $max_width = 280, $max_height = 210)
    {
        $w = imagesx($image); //current width
        $h = imagesy($image); //current height
        if ((!$w) || (!$h)) {
    ...
    private function resize_image_force($image, $width, $height)
    {
        $w = @imagesx($image); //current width
        $h = @imagesy($image); //current height
        if ((!$w) || (!$h)) {
    ...
    $method=force;
     */
    public function ResizeImg($width=280, $height=210, $method = "force")
    {
        $image_loc = $this->img_in;
        $new_loc = $this->img_out;
